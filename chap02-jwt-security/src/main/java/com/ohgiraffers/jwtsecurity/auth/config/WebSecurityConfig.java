...
public class WebSecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain (HttpSecurity http) throws Exception {
        http.csrf(AbstractHttpConfigurer::disable)      // 위조방지
                .addFilterBefore(jwtAuthorizationFilter(), BasicAuthenticationFilter.class)
                .sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))       // 세션을 상태없음으로 관리
                .formLogin(form -> form.disable())      // 시큐리티 컨피그가 제공하는 기본 로그인창을 사용하지 않겠다.
